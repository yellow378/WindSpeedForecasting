...
do  
  ...
  do
    for d_model in 128 512 1024
    do 
      for period_len in  9 18 36
      do  
        # 构建命令字符串
        cmd="python -u run.py \
          --is_training 1 \
        ...
        # 定义日志文件路径
        log_file="logs/LongForecasting/${model_name}_${model_id_name}_${seq_len}_${pred_len}_${model_type}_${d_model}_${period_len}.log"
        # 如果日志文件存在，则删除它
        if [ -f "$log_file" ]; then
            rm "$log_file"
